...
async def query(req: QueryRequest):
    ...
    try:
        ...
        logger.error(f"zhiyan_legal.runner 無法匯入，服務尚未就緒: {ie}")
        raise HTTPException(
            status_code=501,
            detail=f"zhiyan_legal.runner 尚未安裝或無法匯入，請確認 PYTHONPATH 與套件安裝狀態。原始錯誤: {ie}"
        )
...
async def analyze(req: QueryRequest):
    """合約/文件分析（尚未實作）"""
    logger.info(f"Analyze endpoint called but not implemented: prompt={req.prompt[:60]}")
    raise HTTPException(
        status_code=501,
        detail="/analyze 功能開發中，尚未實作。"
    )
