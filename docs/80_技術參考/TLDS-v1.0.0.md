...
| P-03 | `tests/` 建立 `test_tlds_hv.py`：涵蓋 HV-01 至 HV-13 各至少一個單元測試 | Part M | ⬜ 未建立 |
| P-04 | `committee/` 或 `committee_core/` 確認支援 `JUDGE=EXT` 獨立評分角色（FEG_TLDS Layer 2） | §M.3 | ⬜ 待確認 |
| P-05 | `docker/` 確認 LibreOffice headless 版本已安裝，PDF subset embedding 可正常執行 | §N / 規則 D-3 | ⬜ 待確認 |
...
| P-08 | FEG_TLDS 七維度（A–G）完整評分基準表（v1.0.1 工作項，依 §M.3 格式補齊） | §M.3 | ⬜ 規劃中（v1.0.1） |
...
```yaml
tlds_version: "1.0.0"          # str, 必填, SemVer
doc_id: "ZY-LOAN-20260706-0001" # str, 必填, 格式見 §A.2
doc_type: "contract.loan"       # str, 必填, 枚舉見 §A.3
title: "消費借貸契約書"          # str, 必填
version: "1.0.0"                # str, 必填, 文件自身版本
status: "draft"                 # enum: draft | review | final | void
jurisdiction: "TW"              # 固定值, v1.0 僅支援 TW
language: "zh-TW"               # 固定值
confidential: true              # bool, 必填
author: ""                      # str, 必填
reviewer: null                  # str | null
approved_by: null               # str | null
approved_date: null             # ISO 8601 | null
legal_basis: []                 # list[LegalRef], 格式見 §A.4
...
```yaml
- law: "民法"          # 法規名稱，須與知識層法規名稱完全一致
  article: "474"       # 條號（字串，容許 "881-1" 條之體例）
  paragraph: 1         # 項, int | null
  kb_id: "civil-0474"  # zhiyan-legal 知識層節點 ID，必填
```
...
`Noto Serif TC → PMingLiU → 標楷體`（宣告字體名稱不等於嵌入字體檔，合法）。

**規則 D-3**：PDF 輸出必須嵌入字體（subset embedding），確保跨平台列印一致。

> ⚠️ **Pending P-05**：Docker 環境中 LibreOffice headless 版本及 subset embedding 功能尚待確認。
...
deprecated_reason: null # 修法失效時必填，並附新版 clause_id
```
...
生成完成
   ↓
Layer 1｜TLDS-HARD（確定性驗證，二元 PASS/FAIL）
...
| HV-12 | 字體授權合規（無禁用字體進入 PDF 嵌入） | 規則 D-1 |
...
> ⚠️ **Pending P-03**：`tests/test_tlds_hv.py` 尚未建立，HV-01 至 HV-13 單元測試待補。
...
| 版本 | 範圍 | 前置條件 |
|---|---|---|
| v1.0.1 | FEG_TLDS 七維度完整評分基準表 | v1.0 切片跑通 |
...
*TLDS v1.0.0 Draft｜智研 ZhiYan｜MIT License*
